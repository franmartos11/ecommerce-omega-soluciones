...
import { Settings, Globe, ShoppingBag } from "lucide-react";

export default function AdminLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();

  const isConfig = pathname === "/admin";
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden font-sans text-gray-900">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col hidden md:flex">
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          ...
          <Link href="/admin/orders" className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${
          ...
          <Link href="/admin" className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${
            isConfig ? "bg-blue-50 text-blue-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"
          }`}>
